...
const Navbar = () => {
    ...
    return (
        <header className="bg-amber-700 text-white sticky top-0 z-10">
            ...
            <section id="mobile-menu" className={`absolute top-68 bg-black bg-opacity-90 w-full text-5xl flex-col justify-content-center origin-top ${menuOpen ? 'flex animate-open-menu' : 'hidden'}`}>
                ...
                <nav className="flex flex-col min-h-screen items-center py-8" aria-label='mobile'>
                    {links.map((link, index) => (
                        <a key={index} href={link.href} className='w-full text-center p-8 hover:text-amber-600'>
                            <FontAwesomeIcon icon={link.icon} /> {link.text}
                        </a>
                    ))}
                </nav>
